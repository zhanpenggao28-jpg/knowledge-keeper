        ...
        try:
            ...
            if job:
                ...
                try:
                    ...
                    elif task_type == 'ai_classify':
                        _process_ai_classify(item_id)
...
def _process_ai_classify(item_id: str):
    from services.ai_analyzer import classify_and_tag
    ...
    try:
        ...
        if not text.strip():
            conn.execute(
                "UPDATE items SET status = 'done', updated_at = datetime('now') WHERE id = ?",
                [item_id]
            )
            conn.commit()
            return

        result = classify_and_tag(
            text=text,
        ...
        if category in ('document', 'image', 'video', 'other'):
            conn.execute(
                "UPDATE items SET category = ?, updated_at = datetime('now') WHERE id = ?",
                [category, item_id]
            )

        # Auto-title disabled — let user keep original filename
        # Title can still be changed manually or via AI chat rename

        # Apply summary
        summary = result.get('summary', '').strip()
        if summary:
            conn.execute(
                "UPDATE items SET summary = ?, updated_at = datetime('now') WHERE id = ?",
                [summary, item_id]
            )

        # Auto-tagging disabled — let user decide via AI chat instead
        # Tags are still generated by classify_and_tag() and can be used via the chat interface
