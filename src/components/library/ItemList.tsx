...
function ThumbIcon({ item }: { item: Item }) {
  ...
  if (item.category === 'image' && !imgError) {
    return (
      <img src={getFileURL(item.file_path)} alt="" onError={() => setImgError(true)}
        style={{ width: 48, height: 36, objectFit: 'cover', borderRadius: 4 }} />
    )
  }
    ...
    },
    {
      ...
      render: (_: any, record: Item) => (
        <Space size="small">
          ...
          <Button size="small" icon={<FolderOpenOutlined />} onClick={() => window.electronAPI?.openInExplorer(record.file_path)} />
